...
class FeatureGenerator:
    ...
    def __init__(
        self, momentum_periods=[1, 3, 5, 10, 15, 20], vol_periods=[20, 60, 120]
    ):
    ...
    def generate(self, df):
        ...
        for i in self.momentum_periods:
            ...
            if i == 1:
                continue  # Skip EMA for period 1 as it is not meaningful

            df[f"ema_{i}"] = df["Close"].ewm(span=i, adjust=False).mean()

        # Moving average 20 days
        ma20 = df["Close"].rolling(20).mean()
        df["price_ma20_ratio"] = df["Close"] / ma20

        for i in self.vol_periods:
            # Volatility
            df[f"vol_{i}"] = df["ret_1"].rolling(i).std()

            # Volume
            mean = df["Volume"].rolling(i).mean()
            std = df["Volume"].rolling(i).std()

            df[f"volume_ratio_{i}"] = df["Volume"] / df["Volume"].rolling(i).mean()

            df[f"volume_zscore_{i}"] = (df["Volume"] - mean) / std
